...
$workFolder = jpl $here etc "$commandName.Tests.dbops"
$unpackedFolder = Join-Path $workFolder 'unpacked'

$scriptFolder =  Get-Item (cds "$here\etc\install-tests\success")
...
Describe "Add-DBOBuild tests" -Tag $commandName, UnitTests {
    ...
    Context "adding version 2.0 to existing package" {
        ...
        It "build 1.0 should only contain scripts from 1.0" {
            cds 'content\1.0\1.sql' | Should BeIn $results.Path
            cds 'content\1.0\2.sql' | Should Not BeIn $results.Path
        }
        It "build 2.0 should only contain scripts from 2.0" {
            cds 'content\2.0\2.sql' | Should BeIn $results.Path
            cds 'content\2.0\1.sql' | Should Not BeIn $results.Path
        }
        It "should contain module files" {
            cds 'Modules\dbops\dbops.psd1' | Should BeIn $results.Path
            cds 'Modules\dbops\bin\dbup-sqlserver.dll' | Should BeIn $results.Path
            cds 'Modules\dbops\bin\dbup-core.dll' | Should BeIn $results.Path
        }
    ...
    Context "adding new files only based on source path (Type = New)" {
        ...
        It "build 2.0 should only contain scripts from 2.0" {
            cds "content\2.0\$(Split-Path $scriptFolder -Leaf)\2.sql" | Should BeIn $results.Path
            cds "content\2.0\$(Split-Path $scriptFolder -Leaf)\1.sql" | Should Not BeIn $results.Path
        }
        It "should contain module files" {
            cds 'Modules\dbops\dbops.psd1' | Should BeIn $results.Path
            cds 'Modules\dbops\bin\dbup-sqlserver.dll' | Should BeIn $results.Path
    ...
    Context "adding new files only based on hash (Type = Unique/Modified)" {
        ...
        It "build 2.0 should only contain scripts from 2.0" {
            cds "content\2.0\$(Split-Path $scriptFolder -Leaf)\2.sql" | Should BeIn $results.Path
            cds "content\2.0\$(Split-Path $scriptFolder -Leaf)\1.sql" | Should Not BeIn $results.Path
            cds 'content\2.0\Test.sql' | Should Not BeIn $results.Path
        }
        It "build 3.0 should only contain scripts from 3.0" {
            cds 'content\3.0\Test.sql' | Should BeIn $results.Path
            cds "content\3.0\$(Split-Path $scriptFolder -Leaf)\2.sql" | Should Not BeIn $results.Path
            cds "content\3.0\$(Split-Path $scriptFolder -Leaf)\1.sql" | Should Not BeIn $results.Path
        }
        It "should contain module files" {
            cds 'Modules\dbops\dbops.psd1' | Should BeIn $results.Path
            cds 'Modules\dbops\bin\dbup-sqlserver.dll' | Should BeIn $results.Path
